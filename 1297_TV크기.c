...
입력
첫째 줄에 TV의 대각선 길이, TV의 높이 비율, TV의 너비 비율이 공백 한 칸을 사이에 두고 주어진다.
대각선 길이는 5보다 크거나 같고, 1,000보다 작거나 같은 자연수, 높이 비율은 1보다 크거나 같고, 99보다 작거나 같은 자연수 너비 비율은 2보다 크거나 같고,
...
int main() {
	...
	scanf("%f %f %f", &diagonal, &height, &width);
	float x = (float)sqrt(pow(diagonal, 2) / (pow(height, 2) + pow(width, 2)));
	printf("%d %d\n", (int)(height * x), (int)(width * x));
	return 0;
}