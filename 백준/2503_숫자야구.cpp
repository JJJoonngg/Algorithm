...
#include <bits/stdc++.h>
using namespace std;
struct info { string num; int strike, ball; };
vector<info> input;
int n, tmp, Strike, Ball, result = 0, arr[3];
int main() {
	...
	for (int i = 123; i <= 987; i++) {
		...
		for (int j = 0; j < input.size(); j++) {
			Strike = Ball = 0;
			for (int k = 0; k < 3; k++) {
				if (arr[k] == input[j].num[k] - '0') Strike++;
				if (arr[k] == input[j].num[(k + 1) % 3] - '0' || arr[k] == input[j].num[(k + 2) % 3] - '0') Ball++;
			}
			if (input[j].strike == Strike && input[j].ball == Ball) {
				tmp++;
			}
		}
		if (tmp == n) result++;
