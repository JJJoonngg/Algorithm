...
이어서 N장의 색종이에 관한 입력이 각 색종이마다 한 줄씩 차례로 주어진다. 색종이가 놓이는 평면은 가로 최대 101칸, 세로 최대 101칸으로 구성된 격자 모양이다.
...
int main() {
	...
	for (int i = 1; i <= N; i++) {
		cin >> x >> y >> x_ >> y_;
		for (int j = x; j < x + x_; j++) {
			for (int k = y; k < y + y_; k++) {
				papaer[j][k] = i;
			}
		}
	}
