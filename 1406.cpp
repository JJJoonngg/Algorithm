...
둘째 줄에는 입력할 명령어의 개수를 나타내는 정수 N(1≤N≤500,000)이 주어진다. 셋째 줄부터 N개의 줄에 걸쳐 입력할 명령어가 순서대로 주어진다. 
...
int main() {
	...
	while (N--) {
		string order;
		cin >> order;
		if (order == "L" && Left.size() != 0) {
			Right.push(Left.top());
			Left.pop();
		}
		else if (order == "D" && Right.size() !=0) {
			Left.push(Right.top());
			Right.pop();
		}
		else if (order == "B"&& Left.size() != 0) {
			Left.pop();
		}
		else if (order == "P") {
			char insert;
