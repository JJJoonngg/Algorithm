...
이 지도의 위에 주사위가 하나 놓여져 있으며, 주사위의 전개도는 아래와 같다. 지도의 좌표는 (r, c)로 나타내며,
...
가장 처음에 주사위에는 모든 면에 0이 적혀져 있다. 지도의 각 칸에는 정수가 하나씩 쓰여져 있다.
...
#include <stdio.h>
#define MAXI 20 + 1
int N, M, x, y, k, tmp, input;
int map[MAXI][MAXI];
int Dice[6] = { 0, };
void printResult() {
	Dice[5] = tmp;
	if (map[x][y]) Dice[5] = map[x][y], map[x][y] = 0;
	else map[x][y] = tmp;
	printf("%d\n", Dice[2]);
}
int main() {
